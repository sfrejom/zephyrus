...
# 4. Update anchor peers for both organisations.
# ===========================================================================
set -euo pipefail
...
# ---------------------------------------------------------------------------
log_step "Step 4: Updating anchor peers"

# Anchor peer update for Org1.
log_info "Setting anchor peer for SwarmOrg1 ..."
ENV_ARGS=""
for e in "${ORG1_CLI_ENV[@]}"; do ENV_ARGS+=" -e $e"; done

# Use channel fetch + configtxlator approach for Fabric 2.5.
# First, try the simple anchor peer tx if it was generated.
if remote_exec "${UAV2_HOST}" "test -f ~/uav-fabric-network/channel-artifacts/SwarmOrg1MSPanchors.tx" 2>/dev/null; then
    remote_exec "${UAV2_HOST}" "
        docker exec ${ENV_ARGS} cli-org1 peer channel update \
            -o ${ORDERER_HOST}:${ORDERER_PORT} \
            -c ${CHANNEL_NAME} \
            -f ${BASE}/channel-artifacts/SwarmOrg1MSPanchors.tx \
            --tls --cafile ${C_ORDERER_CA} \
        2>&1
    "
    log_info "Anchor peer updated for SwarmOrg1 via transaction file."
else
    log_info "Using configtxlator-based anchor peer update for SwarmOrg1 ..."
    remote_exec "${UAV2_HOST}" "
        docker exec ${ENV_ARGS} cli-org1 bash -c '
            peer channel fetch config config_block.pb \
                -o ${ORDERER_HOST}:${ORDERER_PORT} \
                -c ${CHANNEL_NAME} --tls --cafile ${C_ORDERER_CA}

            configtxlator proto_decode --input config_block.pb --type common.Block \
                | jq \".data.data[0].payload.data.config\" > config.json

            cp config.json modified_config.json
            jq \".channel_group.groups.Application.groups.SwarmOrg1MSP.values += {
                \\\"AnchorPeers\\\": {
                    \\\"mod_policy\\\": \\\"Admins\\\",
                    \\\"value\\\": {
                        \\\"anchor_peers\\\": [{
            ...
            configtxlator compute_update --channel_id ${CHANNEL_NAME} \
                --original config.pb --updated modified_config.pb --output anchor_update.pb

            echo \"{\\\"payload\\\":{\\\"header\\\":{\\\"channel_header\\\":{\\\"channel_id\\\":\\\"${CHANNEL_NAME}\\\",\\\"type\\\":2}},\\\"data\\\":{\\\"config_update\\\":\$(configtxlator proto_decode --input anchor_update.pb --type common.ConfigUpdate | jq .)}}}\" \
                | jq . | configtxlator proto_encode --type common.Envelope --output anchor_update_envelope.pb

            peer channel update -o ${ORDERER_HOST}:${ORDERER_PORT} \
                -c ${CHANNEL_NAME} -f anchor_update_envelope.pb \
                --tls --cafile ${C_ORDERER_CA}

            rm -f config_block.pb config.json modified_config.json config.pb modified_config.pb anchor_update.pb anchor_update_envelope.pb
        ' 2>&1
    "
    log_info "Anchor peer updated for SwarmOrg1 via configtxlator."
fi

sleep 2

# Anchor peer update for Org2.
log_info "Setting anchor peer for SwarmOrg2 ..."
ENV_ARGS=""
for e in "${ORG2_CLI_ENV[@]}"; do ENV_ARGS+=" -e $e"; done

if remote_exec "${UAV3_HOST}" "test -f ~/uav-fabric-network/channel-artifacts/SwarmOrg2MSPanchors.tx" 2>/dev/null; then
    remote_exec "${UAV3_HOST}" "
        docker exec ${ENV_ARGS} cli-org2 peer channel update \
            -o ${ORDERER_HOST}:${ORDERER_PORT} \
            -c ${CHANNEL_NAME} \
            -f ${BASE}/channel-artifacts/SwarmOrg2MSPanchors.tx \
            --tls --cafile ${C_ORDERER_CA} \
        2>&1
    "
    log_info "Anchor peer updated for SwarmOrg2 via transaction file."
else
    log_info "Using configtxlator-based anchor peer update for SwarmOrg2 ..."
    remote_exec "${UAV3_HOST}" "
        docker exec ${ENV_ARGS} cli-org2 bash -c '
            peer channel fetch config config_block.pb \
                -o ${ORDERER_HOST}:${ORDERER_PORT} \
                -c ${CHANNEL_NAME} --tls --cafile ${C_ORDERER_CA}
            ...
            cp config.json modified_config.json
            jq \".channel_group.groups.Application.groups.SwarmOrg2MSP.values += {
                \\\"AnchorPeers\\\": {
                    \\\"mod_policy\\\": \\\"Admins\\\",
                    \\\"value\\\": {
                        \\\"anchor_peers\\\": [{
        ...
        ' 2>&1
    "
    log_info "Anchor peer updated for SwarmOrg2 via configtxlator."
fi
